...
### シナリオ評価（2026-09-17 午後、`tools/eval_agent.py`）

| # | シナリオ | 第 1 回 | 直したこと | 最終 |
|---|---|---|---|---|
| length | 何分？目標との差は？ | （記録漏れ） | eval の server 初期化 | PASS 5.1s |
| longest | 一番長くかける曲 | FAIL 28.5s: get_draft の ms から自分で計算、曲名の表記を変えた | ツール説明とプロンプト（summary の play を引用、表記のまま） | PASS 8.2s |
| structure | 3 曲目のフレーズ構成 | PASS | — | PASS 11.9s |
| hallucination | 無い曲を入れて | PASS（見つからないと答え、Change Set なし） | — | PASS 11.8s |
| fit | 60:00 に収めて | 手選び 4 曲で 106:16（届かず） | `analysis.plan_fit_to_target` を新設 | PASS 23.9s、48 曲で 58:46 |
| remove_one | 1 曲だけ外す | 提案 3 回（op キー無しで 2 回拒否）、差分を引き算 | 操作スキーマ厳密化＋別名許容、diff に増減を明記 | PASS 8.2s |

acid 6/6 PASS（平均 11.5 秒、最大 23.9 秒）、15min_mix 5/5 PASS（不足時の候補出し 27.1 秒、5 曲とも実在）。
途中で「提示してよろしいですか」と聞き返して Change Set を出さない回があり、プロンプトに
「Change Set そのものが確認手順なので聞き返さずに出す」を足して 2 回連続 PASS を確認した。
