...
- `run_setagent.py` — エントリ。既定 WebView2、`--classic`(tkinter)/`--doctor` フォールバック。
...
- dev 起動: リポジトリ直下で `python run_setagent.py`（既定 WebView2）。
- ビルド: `python build.py` → 単一 exe（`SetAgent.exe`）。他人の PC で動く前提。
...
- [ ] `python run_setagent.py` で現行 UI を一度起動し、現状を体感する。
