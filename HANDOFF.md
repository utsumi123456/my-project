...
**未検証 / 次にやること（優先順）:**
1. **frameless にするか（ユーザー判断）** — 自前のドラッグ帯と閉じるボタンが要る。OS のタイトルバーの
   ままなら移動・リサイズ・閉じるは無料で付いてくる。バーは 386px で 2 行に折り返す（98px）— 詰める余地あり。
2. **レコメンド本体（コア価値）** — 現状の入口カードはエージェント（advisor の FIT/FILL 語彙）に
   委ねている。超過時の「外す候補」は advisor が出さない（削り代の案 + 「外すのはお前が決めろ」）。
   ENERGY ピーク/ボトム + `lib.get_play_history` の共起で外す/足す候補を出す機能を `analysis/` 側に。
3. アートワーク表示（曲リストにサムネ）。取得元の調査から。
4. LLM 実キー疎通（ユーザー指示で最後）。
5. `python build.py` で exe 再ビルド（今回の変更は未ビルド）。
...
| 場所 | 中身 |
|---|---|
| PC `C:\Users\7166700\source\setagent\setagent_new\` | **ソースの正本。ここで編集・テスト・ビルドする** |
| PC `C:\Users\7166700\source\setagent\SetAgent_<日付>.zip` | チームに渡すもの（exe ＋ はじめに.md） |
